...
public class DbHandle {
    ...
    /**
     * Search a table with two parameters
     * 
     * @param tbl
     *            Table
     * @param col1
     *            Column for first Value
     * @param val1
     *            first Value
     * @param col2
     *            Column for second Value
     * @param val2
     *            second Value
     * @return a Row or null if nothing was found
     */
    public static <T> Row findUnique(String tbl, String col1, T val1, String col2, T val2) {
	Cursor c;

	try {
	    c = CursorBuilder.createCursor(getTable(tbl));
	    Map<String, T> target = new HashMap<String, T>();
	    target.put(col1, val1);
	    target.put(col2, val2);
	    return (c.findFirstRow(target)) ? c.getCurrentRow() : null;
	} catch (IOException e) {
	    ErrorHandle.popUp("Could not access " + tbl);
	}
	return null;
    }
